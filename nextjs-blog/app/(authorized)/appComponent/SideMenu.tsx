...
export default function SideMenu() {
  ...
  return (
    <div className="flex flex-col gap-4 justify-start items-start mt-20 w-[90%] sm:h-hv text-xl">
      <Link href="/feed">
        <div className="flex h-10 gap-2 justify-start items-center p-2">
          <HouseLine className="text-gray-900  w-6 h-6" />
          <p>Лента</p>
        </div>
      </Link>
      <Link href="/notifications">
        <div className="flex h-10 gap-2 justify-start items-center p-1">
          <Bell className="w-6 h-6 text-yellow-400" /> <p>Уведомления</p>
        </div>
      </Link>

      <Link href={`/user/${userId}`}>
        <div className="flex h-10 gap-2 justify-start p-1 items-center">
      ...
      <button
        ...
        className="flex bg-gray-800 text-white rounded-sm w-[100%] h-12 gap-2 p-2 justify-start items-center"
      >
